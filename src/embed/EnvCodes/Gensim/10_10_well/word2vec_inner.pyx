...
cdef unsigned long long w2v_fast_sentence_cbow_neg(
    ...
    const np.uint32_t target_negative_indices[100][1000],
    const np.uint32_t randomIdx) nogil:
    """Train on a single effective word from the current batch, using the CBOW method.
    ...
    for d in range(negative+1):
        if d == 0:
            target_index = word_index
            label = ONEF
        elif (randomIdx != <np.uint32_t>-1) and (count_resistant < 10):
            target_index = target_negative_indices[i][count_resistant]
            #printf("target_index : %d, i: %d, word_index : %d, randIdx :%d \n", target_index, i, word_index, randomIdx)
            label = <REAL_t>0.0
            count_resistant += 1
        else:
            target_index = bisect_left(cum_table, (next_random >> 16) % cum_table[cum_table_len-1], 0, cum_table_len)
...
def train_batch_cbow(model, sentences, alpha, _work, _neu1, compute_loss):
    ...
    cdef int effective_words = 0, effective_sentences = 0
    cdef int sent_idx, idx_start, idx_end
    cdef int cnt = 0, randnum, rand_value
    ...
    for sent in sentences:
        ...
        for token in sent:
            ...
            if c.hs:
                ...
                c.points[effective_words] = <np.uint32_t *>np.PyArray_DATA(word.point)
            if c.negative:
                c.target_negative_indices_random[effective_words] = -1
                if word.index in model.target_indices:
                    word_negative_indices = model.target_negative_indices[np.where(model.target_indices == word.index)]
                    #print('word.index', word.index, 'word.resistant count', len(word_negative_indices[0]))
                    if len(word_negative_indices[0]) <= 0:
                        c.target_negative_indices_random[effective_words] = -1
                    else:
                        temprand = len(word_negative_indices[0])
                        #print('word.index ', word.index, 'word.resistant count ', len(word_negative_indices[0]), 'temprand ', temprand)
                        if temprand <= 0:
                            c.target_negative_indices_random[effective_words] = -1
                            continue
                        randnum = rand.randrange(temprand)
                        c.target_negative_indices_random[effective_words] = randnum
                        #print('word.index ', word.index, 'word.resistant count ', len(word_negative_indices[0]), 'temprand ', temprand, 'randnum ', randnum)
                        #printf("c.target_negative_indices_random : %d\n", c.target_negative_indices_random[effective_words])
                        for cnt in range(10):
                            rand_value = randnum + cnt
                            if (rand_value >= temprand):
                                rand_value = randnum + cnt - temprand
                            c.target_negative_indices[effective_words][cnt] = word_negative_indices[0][rand_value]
                            # printf("effectiveword %d , cnt %d, randnum %d, ", effective_words, cnt, randnum)
                            # printf(":c.target_negative_indices[%d][%d]: %d\n", effective_words, cnt, c.target_negative_indices[effective_words][cnt])
            effective_words += 1
            if effective_words == MAX_SENTENCE_LEN:
                break  # TODO: log warning, tally overflow?
    ...
    with nogil:
        for sent_idx in range(effective_sentences):
            ...
            for i in range(idx_start, idx_end):
                ...
                if c.negative:
                    c.next_random = w2v_fast_sentence_cbow_neg(c.negative,
                                                               c.cum_table,
                                                               c.cum_table_len,
                                                               c.codelens,
                                                               c.neu1,
                                                               ...
                                                               c.target_negative_indices,
                                                               c.target_negative_indices_random[i])
