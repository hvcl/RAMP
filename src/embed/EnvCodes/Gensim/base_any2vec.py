...
class BaseWordEmbeddingsModel(BaseAny2VecModel):
    ...
    def __init__(self, sentences=None, corpus_file=None, workers=3, vector_size=100, epochs=5, callbacks=(),
                 ...
                 ns_exponent=0.75, cbow_mean=1, min_alpha=0.0001, compute_loss=False, negative_list=None,
        ...
        **kwargs : object
        ...
        self.target_indices = list()  # np.array(list())
        self.target_negative_indices = list()  # np.array(list())
        ...
        if sentences is not None or corpus_file is not None:
            ...
            if negative_list != None:
                self._fill_negative_indices(negative_list)
            self.train(
    ...
    def _fill_negative_indices(self, negative_list):
        target_vocabs = list()

        for word, word_negative_list in negative_list.items():
            target_vocabs.append(self.wv.vocab[word])
            self.target_negative_indices.append(np.array([self.wv.vocab[w].index for w in word_negative_list]))

        self.target_indices = np.array([_.index for _ in target_vocabs])
        self.target_negative_indices = np.array(self.target_negative_indices)
